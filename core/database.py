...
DATABASE_FILE = os.path.join(BASE_DIR, "data", "bot_data.db")
...
RED_CORE_MINE_CONFIG = {
    "warm": {
        "title": "Тёплый разлом",
        "shard_cost": 3,
        "coin_cost": 150,
        "cooldowns": {"normal": 600, "vip": 480, "vip_plus": 360},
        "success_chance": 0.80,
        "success_cores": (1, 1),
        "success_coins": (300, 450),
        "success_rating": 2,
        "fail_rating": 1,
        "fail_refund": (0, 1),
        "fail_refund_chance": 0.40,
    },
    "deep": {
        "title": "Глубинная жила",
        "shard_cost": 7,
        "coin_cost": 500,
        "cooldowns": {"normal": 1500, "vip": 1200, "vip_plus": 900},
        "success_chance": 0.62,
        "success_cores": (2, 3),
        "success_coins": (900, 1400),
        "success_rating": 5,
        "fail_rating": 2,
        "fail_refund": (1, 2),
        "fail_refund_chance": 1.0,
    },
    "heart": {
        "title": "Сердце пласта",
        "shard_cost": 15,
        "coin_cost": 1500,
        "cooldowns": {"normal": 3600, "vip": 2700, "vip_plus": 2100},
        "success_chance": 0.42,
        "success_cores": (5, 7),
        "success_coins": (2500, 4200),
        "success_rating": 12,
        "fail_rating": 3,
        "fail_refund": (3, 5),
        "fail_refund_chance": 1.0,
        "luck_coupon_chance": 0.08,
    },
}

RED_CORE_EXCHANGE_CONFIG = {
    "coins": {"title": "Продать 1 ядро", "core_cost": 1, "coins": 750},
    "search_skip": {"title": "Сбросить поиск", "core_cost": 3, "last_search": 0},
    "fragment": {"title": "Фрагмент Селюка", "core_cost": 4, "selyuk_fragments": 1},
    "luck": {"title": "Купон удачи", "core_cost": 6, "luck_coupon_charges": 1},
    "auto3": {"title": "Автопоиск x3 на 24ч", "core_cost": 10, "auto_search_boost_count": 3, "auto_search_boost_seconds": 24 * 60 * 60},
}
...
class Player(Base):
    ...
    red_cores = Column(Integer, default=0)
    last_red_core_run = Column(Integer, default=0)
    red_core_runs = Column(Integer, default=0)
    red_core_successes = Column(Integer, default=0)
...
def get_red_core_cooldown(mode: str, is_vip: bool = False, is_vip_plus: bool = False) -> int:
    cfg = RED_CORE_MINE_CONFIG.get(str(mode or ""))
    if not cfg:
        return 0
    key = "vip_plus" if is_vip_plus else "vip" if is_vip else "normal"
    return int(cfg["cooldowns"][key])


def _red_core_roll_value(roll, key: str, default):
    if isinstance(roll, dict) and key in roll:
        return roll[key]
    if key == "success_roll" and isinstance(roll, (int, float)):
        return float(roll)
    return default() if callable(default) else default


def apply_red_core_mine_atomic(
    user_id: int,
    mode: str,
    now_ts: int,
    is_vip: bool,
    ...
    """Атомарная добыча Красных ядер земли."""
    mode = str(mode or "")
    cfg = RED_CORE_MINE_CONFIG.get(mode)
    if not cfg:
    ...
    try:
        ...
        cooldown = get_red_core_cooldown(mode, is_vip=bool(is_vip), is_vip_plus=bool(is_vip_plus))
        last_run = int(getattr(player, "last_red_core_run", 0) or 0)
        elapsed = now - last_run
        if last_run and elapsed < cooldown:
            try:
                dbs.rollback()
            except Exception:
                pass
            return {
                "ok": False,
                "reason": "cooldown",
                "time_left": int(cooldown - elapsed),
                "cooldown": int(cooldown),
                "mode": mode,
            }

        shard_cost = int(cfg["shard_cost"])
        coin_cost = int(cfg["coin_cost"])
        ...
        if current_shards < shard_cost:
            try:
                ...
                pass
            return {"ok": False, "reason": "not_enough_shards", "need": shard_cost, "have": current_shards, "mode": mode}
        if current_coins < coin_cost:
            try:
                dbs.rollback()
            except Exception:
                pass
            return {"ok": False, "reason": "not_enough_coins", "need": coin_cost, "have": current_coins, "mode": mode}

        success_roll = float(_red_core_roll_value(roll, "success_roll", random.random))
        success = success_roll < float(cfg["success_chance"])
        cores_delta = 0
        coins_delta = 0
        rating_delta = int(cfg["success_rating"] if success else cfg["fail_rating"])
        shard_refund = 0
        luck_coupon_delta = 0

        if success:
            cores_min, cores_max = cfg["success_cores"]
            coins_min, coins_max = cfg["success_coins"]
            cores_delta = int(_red_core_roll_value(roll, "cores", lambda: random.randint(int(cores_min), int(cores_max))))
            coins_delta = int(_red_core_roll_value(roll, "coins", lambda: random.randint(int(coins_min), int(coins_max))))
            if mode == "heart":
                luck_roll = float(_red_core_roll_value(roll, "luck_roll", random.random))
                if luck_roll < float(cfg.get("luck_coupon_chance", 0.0)):
                    luck_coupon_delta = 1
        else:
            refund_chance = float(cfg.get("fail_refund_chance", 1.0))
            refund_roll = float(_red_core_roll_value(roll, "refund_roll", random.random))
            if refund_roll < refund_chance:
                ref_min, ref_max = cfg["fail_refund"]
                shard_refund = int(_red_core_roll_value(roll, "refund", lambda: random.randint(int(ref_min), int(ref_max))))

        player.power_shards = max(0, current_shards - shard_cost + shard_refund)
        player.coins = current_coins - coin_cost + coins_delta
        player.rating = int(getattr(player, "rating", 0) or 0) + rating_delta
        player.red_cores = int(getattr(player, "red_cores", 0) or 0) + cores_delta
        player.last_red_core_run = now
        player.red_core_runs = int(getattr(player, "red_core_runs", 0) or 0) + 1
        if success:
            player.red_core_successes = int(getattr(player, "red_core_successes", 0) or 0) + 1
        if luck_coupon_delta:
            player.luck_coupon_charges = int(getattr(player, "luck_coupon_charges", 0) or 0) + luck_coupon_delta
        ...
        result = {
            ...
            "shard_refund": int(shard_refund),
            "cores_delta": int(cores_delta),
            "coins_delta": int(coins_delta),
            "rating_delta": int(rating_delta),
            "luck_coupon_delta": int(luck_coupon_delta),
            "power_shards_after": int(player.power_shards or 0),
            "red_cores_after": int(player.red_cores or 0),
            "coins_after": int(player.coins or 0),
            "rating_after": int(player.rating or 0),
            "last_red_core_run": int(player.last_red_core_run or 0),
            "red_core_runs": int(player.red_core_runs or 0),
            "red_core_successes": int(player.red_core_successes or 0),
            "username": getattr(player, "username", None),
        }
        dbs.commit()
        try:
            status = "success" if success else "fail"
            log_action(int(user_id), result.get("username"), "power_red_cores", f"mine:{mode}:{status}", amount=coins_delta - coin_cost, success=True)
        except Exception:
            pass
        return result
    except Exception as e:
        try:
            ...
            pass
        logger.error(f"apply_red_core_mine_atomic failed for user {user_id}: {e}")
        return {"ok": False, "reason": "exception"}
    finally:
        dbs.close()


def exchange_red_cores_atomic(user_id: int, exchange_key: str) -> dict:
    """Атомарный обмен Красных ядер на награды."""
    exchange_key = str(exchange_key or "")
    cfg = RED_CORE_EXCHANGE_CONFIG.get(exchange_key)
    if not cfg:
        return {"ok": False, "reason": "bad_exchange"}

    dbs = SessionLocal()
    try:
        _begin_write_transaction(dbs)
        player = _get_or_create_player_in_session(dbs, int(user_id))
        core_cost = int(cfg["core_cost"])
        current_cores = int(getattr(player, "red_cores", 0) or 0)
        if current_cores < core_cost:
            try:
                dbs.rollback()
            except Exception:
                pass
            return {"ok": False, "reason": "not_enough_cores", "need": core_cost, "have": current_cores, "exchange_key": exchange_key}

        now = int(time.time())
        rewards: dict[str, int] = {}
        player.red_cores = current_cores - core_cost

        if "coins" in cfg:
            value = int(cfg["coins"])
            player.coins = int(getattr(player, "coins", 0) or 0) + value
            rewards["coins"] = value
        if "last_search" in cfg:
            player.last_search = int(cfg["last_search"])
            rewards["last_search"] = int(cfg["last_search"])
        if "selyuk_fragments" in cfg:
            value = int(cfg["selyuk_fragments"])
            player.selyuk_fragments = int(getattr(player, "selyuk_fragments", 0) or 0) + value
            rewards["selyuk_fragments"] = value
        if "luck_coupon_charges" in cfg:
            value = int(cfg["luck_coupon_charges"])
            player.luck_coupon_charges = int(getattr(player, "luck_coupon_charges", 0) or 0) + value
            rewards["luck_coupon_charges"] = value
        if "auto_search_boost_count" in cfg:
            count = int(cfg["auto_search_boost_count"])
            seconds = int(cfg["auto_search_boost_seconds"])
            current_until = int(getattr(player, "auto_search_boost_until", 0) or 0)
            player.auto_search_boost_count = int(getattr(player, "auto_search_boost_count", 0) or 0) + count
            player.auto_search_boost_until = max(current_until, now) + seconds
            rewards["auto_search_boost_count"] = count
            rewards["auto_search_boost_until"] = int(player.auto_search_boost_until)

        result = {
            "ok": True,
            "exchange_key": exchange_key,
            "title": cfg["title"],
            "core_cost": int(core_cost),
            "red_cores_after": int(player.red_cores or 0),
            "coins_after": int(getattr(player, "coins", 0) or 0),
            "power_shards_after": int(getattr(player, "power_shards", 0) or 0),
            "rewards": rewards,
            "username": getattr(player, "username", None),
        }
        dbs.commit()
        try:
            log_action(int(user_id), result.get("username"), "power_red_cores", f"exchange:{exchange_key}", amount=core_cost, success=True)
        except Exception:
            pass
        return result
    except Exception as e:
        try:
            ...
            pass
        logger.error(f"exchange_red_cores_atomic failed for user {user_id}: {e}")
        return {"ok": False, "reason": "exception"}
    finally:
        dbs.close()
...
def ensure_schema():
    ...
    with engine.begin() as conn:
        ...
        if 'red_cores' not in cols:
            conn.exec_driver_sql("ALTER TABLE players ADD COLUMN red_cores INTEGER DEFAULT 0")
        if 'last_red_core_run' not in cols:
            conn.exec_driver_sql("ALTER TABLE players ADD COLUMN last_red_core_run INTEGER DEFAULT 0")
        if 'red_core_runs' not in cols:
            conn.exec_driver_sql("ALTER TABLE players ADD COLUMN red_core_runs INTEGER DEFAULT 0")
        if 'red_core_successes' not in cols:
            conn.exec_driver_sql("ALTER TABLE players ADD COLUMN red_core_successes INTEGER DEFAULT 0")
...
def sell_all_drinks_of_rarity(user_id: int, rarity: str) -> dict:
    """Массовая продажа всех энергетиков указанной редкости.
    Возвращает dict: {ok, total_items_sold, total_earned, items_processed, coins_after, unit_payout}
    """
    dbs = SessionLocal()
    try:
        user_items = (
            dbs.query(InventoryItem)
            .filter(InventoryItem.player_id == user_id, InventoryItem.rarity == rarity)
            .all()
        )
        if not user_items:
            return {"ok": False, "reason": "no_items"}
        ...
        rating_value = int(getattr(player, 'rating', 0) or 0)
        unit_payout = get_receiver_unit_payout_with_rating(rarity, rating_value)
        if unit_payout <= 0:
            return {"ok": False, "reason": "unsupported_rarity"}
