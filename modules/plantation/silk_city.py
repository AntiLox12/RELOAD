...
def create_plantation(user_id: int, username: str, investment_level: str, plantation_name: str = None) -> Dict:
    ...
    try:
        ...
        # Оптимизированная проверка VIP статуса игрока без создания новых сессий (предотвращает откат транзакции SQLite в тестах)
        is_vip = bool(
            (player.vip_until and current_time < int(player.vip_until)) or 
            (player.vip_plus_until and current_time < int(player.vip_plus_until))
        )
