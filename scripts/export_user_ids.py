...
try:
    ...
    from core.database import SessionLocal, Player  # type: ignore
except Exception as e:
    print("[ERROR] Не удалось импортировать модуль core.database.py.")
    raise
...
def main(argv: list[str] | None = None) -> int:
    ...
    default_output = os.path.join(BASE_DIR, 'data', 'user_ids.csv')
    parser = argparse.ArgumentParser(description="Экспорт всех user_id из базы данных в файл")
    parser.add_argument('--output', '-o', default=default_output, help='Путь к выходному файлу (по умолчанию data/user_ids.csv)')
